...
<template>
  ...
  >
    ...
    <div class="relative flex items-center gap-1.5 shrink-0">
      <div
        class="flex shrink-0 items-center justify-center rounded-md bg-blue-500 font-bold text-white"
    ...
    <div class="relative my-auto" :class="size === 'sm' ? 'py-2' : 'py-3'">
      <div v-if="tags?.length" class="mb-1.5 flex flex-wrap gap-1">
        <span
          ...
          :key="tag"
          class="rounded-full bg-blue-500/15 font-medium text-blue-300"
    ...
    >
      ...
      <div
        ...
        class="flex shrink-0 items-center justify-center rounded-full bg-blue-500/30 font-bold text-blue-300"
      ...
      <div class="min-w-0">
        ...
        <p
          v-if="size !== 'sm'"
          class="truncate text-slate-500 text-[10px]"
        >{{ author.role }}</p>
      </div>
    </div>
  </div>
